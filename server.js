...
app.get("/obtener-grados-profesor", async (req, res) => {
  ...
  try {
    // Llamar a la función SQL personalizada para obtener los grados únicos
    const { data: grados, error } = await supabase
      .rpc("obtener_grados_profesor_distinct", { profesor_id: profesorId });

    if (error) {
      console.log("Error al obtener grados:", error);
      return res.status(500).json({ error: "Error al obtener grados." });
    }
    
    console.log(grados); // Imprimir grados obtenidos
